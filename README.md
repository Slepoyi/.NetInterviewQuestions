...
<details>
    <summary>Можно ли разместить объект ссылочного типа на стеке, а значимого в куче?</summary>

Да, можно в обе стороны.

Размещение значимого типа в куче - боксинг (приведение к object) или помещение в поле класса (класс со своими значениями выделяется в куче).

Разместить ссылочный тип на стеке вручную можно только махинациями с unsafe-кодом, когда мы вручную выделяем память под объект и раскладываем по байтам нужные значения (под блок синхронизации, метаданные, сами значения).

Еще ссылочные типы могут быть размещены на стеке вследствие escape analysis в JIT. Компилятор анализирует скоуп жизни объекта и в случае, когда объект создаётся и используется в скоупе текущего метода (не выходит за его пределы) может оптимизировать его расположение.
...
<details>
<summary>
    Иерархия обобщенных коллекций в C#
</summary>

```IEnumerable```
Позволяет итерироваться по коллекции. Доступ readonly, модифицировать коллекцию нельзя. Информации о размере коллекции нет, потенциально коллекция может быть бесконечной.
Важной концепцией является работа с LINQ без материализации. Данные обрабатываются только тогда, когда были запрошены. Вследстве этого многократные итерации по коллекции будут медленными и могут получать разные результаты.
Нужно, когда нам требуется только итерация по коллекции.

```IReadOnlyCollection```
К итерации добавляется информация о размере коллекции - свойство Count.
Нужно, когда мы отдаём вызывающему коду коллекцию только для чтения с заранее известным размером (материализованную).

```ICollection```
К IReadOnlyCollection добавляются методы записи (Add, Remove).
Нужно, когда вызывающий код собирается менять коллекцию.

```IList```
К методам ICollection добавляется доступ к элементу по индексу в коллекции.
Нужно, когда вызывающий код собирается обращаться к элементам по индексу (например, in-place алгоритмы)

```IDictionary```
Коллекция пар ключ-значение с индексатором по ключу, свойствами Keys и Values + метод TryGetValue.
Когда требуется быстрый доступ по уникальному ключу.

Соответственно, в зависимости от конкретного интерфейса и реализации коллекции под ним имеем различный набор доступных операций и их алгоритмическую сложность.

```IEnumerable<T>```
Итерация, поиск, определение размера - O(n).

```ICollection<T>```
Итерация, поиск, удаление - O(n). Вставка - O(1) или O(n) в зависимости от реализации. Определение размера - O(1).

```IList<T>```
Итерация, поиск, удаление - O(n). Вставка - O(1) или O(n) при необходимости расширения внутреннего массива. Определение размера, доступ по индексу - O(1).

Конкретные реализации
```Array<T>```
Массив элементов с фиксированной длиной. Нужен, когда мы знаем, что длина не изменится. Чуть бюолее оптимизирован, чем List.

```List<T>```
Массив с переменной длиной. Основные операции - добавление в конец и перебор. Как и у Array, Contains по коллекции имеет сложность O(n).

```HashSet<T>```
Набор уникальных значений. Быстрый поиск - Contains даёт O(1). 
Накладывает на тип T два ограничения:
1) Корректный GetHashCode
2) Корректный Equals (или реализация IEquatable)

По сути, нужны типы, у которых семантика сравнения как у value-типов (int, string, record)

```Dictionary<TKey, TValue>```
Набор пар ключ-значение с уникальным ключом. Применение - кэширование, индексация.
Ограничения на TKey такие же, как и у HashSet на T.

```Queue<T>```
Очередь, FIFO семантика. Используется для буферов, очередей задач и т.п.

```Stack<T>```
Стек, LIFO семантика. undo/redo, обход в глубину, парсинг.

```LinkedList<T>```
Коллекция, в которой элементы связаны друг с другом по одному. Нужно, когда необходимы частые вставки в середину. Используется редко.

```Frozen коллекции```
Используются, когда коллекция создаётся 1 раз и используется для множества чтений в будущем. Под капотом в зависимости от структуры данных выбирается наиболее оптимальная стратегия хранения и поиска.
Чтение из FrozenDictionary быстрее, чем из обычного Dictionary, при этом создание коллекции будет дороже.

**Главное правило: сигнатуры методов принимают минимально возможный интерфейс, возвращают максимально конкретный.**
</details>
...
<details>
<summary>
    Что такое кардинальность в SQL, зачем нужна и на что влияет
</summary>
    
Кардинальность - это понятие, отражающее количество столбцов, возвращаемое каким-либо предикатом.
Приблизительные значения
До 1000 строк - низкая кардинальность
От 1000 до 100_000 - средняя кардинальность
От 100_000 - высокая кардинальность

Кардинальность используется сервером SQL для выбора оптимального плана выполнения.
1) Порядок соединения таблиц (таблицы с низкой кардинальностью соединяются первыми)
2) Метод доступа к данным - при низкой кардинальности наиболее вероятно использование **seek** некластеризованных индексов, для высокой - **scan** кластеризованного индекса (прямой просмотр таблицы)
3) Выбор метода соединения - при низкой кардинальности используется **nested loops**, при высокой - **merge** или **hash**
4) Оценка выделения памяти (при неверной оценке кардинальности могут произойти записи данных в базу **tempdb** вместо удержания всего набора данных в памяти, что ведет к замедлению, т.к. IO при работе с диском медленнее, чем при работе в памяти)
5) Параллелизм выполнения - как правило не используется при низкой кардинальности 
...
Транзакция - это единица работы, выполняемая бд. Все операции в транзакции либо выполняются полность, либо не выполняются. Являются одним из инструментов удовлетворения **ACID** в бд.

Чтобы начать транзакцию, нужна команда ```BEGIN TRANSACTION```. Для сохранения результатов транзакции нужна команда ```COMMIT TRANSACTION```. 

Для подсчета числа активных транзакций используется переменная ``TRANCOUNT``

В случае ошибки транзакцию нужно откатить командой ```ROLLBACK TRANSACTION```. Если этого не сделать, транзакция не закроется и будет оставаться открытой какое-то время (скорее всего, пока живо подключение к бд).

Транзакции поддерживают так называемые точки сохранения. Для создания точки сохранения требуется команда ```SAVE TRANSACTION SavePointName```. В таком случае, если в последующем ходе выполнения транзакции возникнет ошибка, можно будет откатить транзакцию не целиком, а к точке сохранения с помощью ```ROLLBACK TRANSACTION SavePointName``, что позволит не потерять все изменения, созданные до нее.

Точки сохранения можно использовать в случае, если разные части транзакции имеют разные условия ошибки, а также для больших транзакций.

SQL Server поддерживает вложенные транзакции. Это означает, что внутри одной транзакции можно начать другую. Однако, они не работают так, как того можно было бы ожидать. Вложенные ```COMMIT``` лишь уменьшают число ```TRANCOUNT```, все изменения записываются только после выполнения последнего ```COMMIT```, а любой ```ROLLBACK``` откатывает весь стэк транзакций целиком.
...
<details>
<summary>
    Пессимистичные блокировки в SQL Server
</summary>

Практически все операции над таблицами в SQL Server используют блокировки.

**X** - (eXclusive lock) - монопольные блокировки. Монопольные блокировки запрещают другим транзакциям блокировать даный объект до своего завершения. Обычно используются для модификации данных.

**U** - (Update lock) - блокировка обновления. Данный тип блокировок используется, когда SQL SERVER ищет строку для обновления. Он проверяет, удовлетворяет ли строка условию из запроса. Если да, то блокировка меняется на монопольную (X), иначе блокировка обновления снимается. 

**S** (Shared lock) - совмещаемые блокировки. Эти блокировки возникают, когда происходит запрос на чтение данных (например, в READ COMMITTED транзакции) и обычно (но не всегда) снимаются сразу после чтения. Несколько транзакций могут совмещать данные блокировки.

Поведение совмещаемых блокировок для различных уровней изоляции:
- NOLOCK - блокировки не устанавливаются
- READCOMMITTED - блокировки устанавливаются и сразу снимаются (после SELECT)
- REPEATABLEREAD/SERIALIZABLE - блокировки устанавливаются и держатся до конца транзакции

Также существуют намерения блокировки (префикс I). Они указывают на то, что планируется осуществление блокировки над дочерними ресурсами объекта.

**IS** - чтение дочерних ресурсов.

**IU** - обновление дочерних ресурсов.

**IX** - изменение дочерних ресурсов.

**Проблемы**
При блокировках может возникать множество проблем, вытекающих из неправильного использования.
Например, если обновлять данные в таблице, основываясь на столбце, по которому нет индекса, SQL SERVER вынужден сканировать всю таблицу и устанавливать блокировки обновления (U) на каждую строку, которую прочитал.

**Совместимость блокировок**
Блокировки с намерениями (IU/IX) совместимы друг с другом.
Монопольные блокировки (X) несовместимы друг с другом.
Блокировки обновления (U) несовместимы друг с другом, а также с монопольными блокировками (X).
Совмещаемые блокировки (S/IS) совместимы друг с другом, а также с блокировкой обновления (U/IU). При этом блокировка (IS) совместима с (IX), но несовместима с (X), а (S) несовместима с (X/IX).

**Объекты блокировок**
Заблокирована может быть:
- база данных (возникает при бэкапах/восстановлении)
- таблица (массовые обновления)
- страница таблицы (при обновлении по решению SQL SERVER)
- строка таблицы (обновление конкретной строки по предикату)
- ключ (операции, затрагивающие индекс)

**Блокировки диапазона**
Блокировки диапазона (Shared) - возникают на уровне SERIALIZABLE и блокируют диапазон значений индекса даже в случае отсутствия значения в каком-либо промежутке.
Например, при работе с диапазоном id (1,2,3,5,6) значение 4 будет заблокировано, несмотря на его отсутствие в таблице.
Виды:
- RangeS-S (Range Shared-Shared) - SELECT в SERIALIZABLE
- RangeS-U (Range Shared-Update) - SELECT WITH (UPDLOCK) в SERIALIZABLE
- RangeX-X (Range Exclusive-Exclusive) - INSERT/UPDATE/DELETE в SERIALIZABLE

**Эскалация блокировок**
Эскалация блокировок - это механизм замены множества мелких блокировок одной более крупной.
Эскалация чаще всего возникает при превышении порога блокировок (~5000 на один объект. Например, обновление по широкому диапазону. SQL SERVER начинает с накладывания ROWLOCK, но при превышении порога эскалирует блокировку до TABLOCK.
При необходимости массового обновления, которое почти наверняка вызовет эскалацию, рекомендуется использовать батчинг.

</details>

<details>
<summary>
    Оптимистичные блокировки в SQL Server
</summary>
Различие с пессимистичными:
    
Пессимистичные блокировки:
"Я читаю данные и блокирую их, потому что уверен, что кто-то захочет их изменить"

Оптимистичные блокировки:
"Я читаю данные без блокировок, а перед изменением проверяю, не изменил ли их кто-то другой"

В SQL SERVER есть специальный тип ROWVERSION, который изменяется при каждом UPDATE строки. 
В бд есть счетчик, который инкрементируется ппри каждой операции вставки/обновления над таблицей, в которой есть столбец rowversion. Этот счетчик является источником уникальных чисел для rowversion.

Суть оптимистичной блокировки в том, что инструкция обновления становится такой:

```
UPDATE Products 
SET Price = 100
WHERE ProductID = 1 AND Version = 0x07D1;
```

После обновления Version данной строки изменится.

Следовательно, если 2 потока прочитали одинаковый rowversion для одной и той же строки, то только 1, самый быстрый, сможет обновить данные. Для второго @@ROWCOUNT = 0, что необходимо отслеживать на уровне приложения и использовать политику ретраев, выброс исключений, возврат ошибки и т.д.

Также существует SNAPSHOT. Исходя из названия, использует слепок данных на конкретный момент времени. Защищает от фантомов, как и SERIALIZABLE, но использует не пессимистичное блокирование, а версионирование строк в tempdb.
SNAPSHOT должен быть включен на уровен бд, также требует дополнительное место для хранения версионированных строк.
Положительным моментом является то, что SNAPSHOT не блокирует читателей и писателей, так как работает со своим слепком данных.
</details>
...
6. Аккуратно включать в индекс тяжелые поля, так как это ведет к раздуванию индекса

**Примеры запросов и индексов**
1)
where a = 1 and b > 10 order by c

Тут есть равенство - первым идет равенство. Затем сортировка, после нее диапазон.
Хорошие возможные индексы: (a, c, b), (a, b, c)
Первый индекс хорош, когда по набору (a, b) остаётся много строк. После этого придется сортировать всё по c (так как индекс держит c упорядоченными после b), и если c идет первым, то данные уже отсортированы. Если первым был бы b, то данные по c отсортированы для каждого b, и их бы пришлось дорого пересортировывать.

Однако, если индекс по диапазону очень селективный и оставит после себя несколько строк, то их сортировка будет дешевой и лучше выбрать индекс (a, b, c) (или вообще создать покрывающий индекс (a, b) include (c)).

2)
where b > 10 order by c

Если фильтр по b возвращает много строк (~30%), то при наличии индекса (c, b) мы избегаем дорогой сортировки путем последовательного чтения уже отсортированных данных с фильтрацией.
Если бы был индекс (b, c) то нам бы пришлось сортировать 30% строк, что плохо.
Однако, если фильтр по b возвращает мало строк (1-5%), то лучше делать фильтр сначала по нему ценой сортировки небольшого количества строк.

3)
select d from table where b > 10
и у нас есть индекс (b)

Предположим, что индекс по b обладает малой селективностью. Тогда сначала потребуется прочитать 500_000 строк из индекса, затем прочитать d из таблицы (500_000 чтений). Итого 1_000_000 случайных чтений - очень медленно.
В таком случае оптимизатор скорее решит читать таблицу последовательно, и уже там фильтровать и сортировать.
Однако, если индекс будет покрывающим (b) include (d), то здесь 500_000 последовательных чтений индекса, что будет проще и оптимизатор выберет это.

4) 
SELECT 
    a,
    COUNT(a) as cnt,
    RANK() OVER (ORDER BY COUNT(a) DESC) as rnk
FROM table
WHERE a > 10
GROUP BY a

Здесь очевидно поможет индекс по (a). Так как он есть, то группировка (выбор уникальных значений) пройдет быстро, без создания отдельной хэш-таблицы.
Однако, COUNT(a) в оконной функции - операция над уже сгруппированными данными, которая выполняется в памяти. Она не использует индекс.
...
Здесь у нас происходит join с сортировкой по полю одной из таблиц.
Возможные стратегии join:
a) nested loops + sort - сканирует t1, для каждой строки ищет совпадение в t2, потом сортирует
Выбирается для маленьких таблиц. Для t1 индекс не поможет, для t2 подойдет индекс на t1_id
b) merge join - сортирует t1 по id и t2 по t1_id, затем сливает результат.
Выбирается, когда таблицы большие и нужны отсортированные данные. Подойдут индексы на t1 по id, на t2 по t1_id
c) hash join + sort - строит хэш-таблицу по одной из таблиц (обычно меньшей по размеру), допустим t1, затем сканирует t2 и выбирает совпадения, после чего сортирует.
Выбирается, когда таблицы большие и нет порядка. Подойдет индекс на t2 по (t1_id) include (b)

Итого, наиболее универсальные индексы
t1: (a, id)
t2: t1_id include (b)

Индекс по первой таблице позволяет не сортировать данные после джоина для nested loops, для merge же нужна сортировка по id, после джоина придется пересортировать.
Индекс по второй таблице покрывает все варианты использования.

Если по таблице t1 выбрать индекс (id, a), то мы ускорим merge join, но в конце всё равно придется сортировать данные по a.
...
Для t1 в случае, если фильтр по id возвращает мало строк, лучше индекс (id) include (a), так как джоин будет происходить на микро-наборе данных. 
Для t2 производится сортировка по столбцу b. Следовательно, необходим индекс (t1_id, b).
Индекс подходит, потому что сначала происходит джоин и только потом сортировка. Сначала нужно пройтись по совпадениям в t1_id, только затем упорядочивание по b.
...
В случае пагинации по большому объему с сортировкой скорее всего планировщик выберет в качестве ведущей таблицы t2.
На t2 нужен индекс (b, t1_id) для того, чтобы можно было читать t2 в порядке сортировки по b и брать t1_id.
На t1 нужен индекс по (id), скорее всего кластеризованный.
...
<details>
<summary>
    Что такое left semi join в SQL SERVER
</summary>
    
Left semi join - это соединение, при котором возвращаются только столбцы из левой таблицы вне зависимости от числа совпадений в правой таблице.

```
SELECT Name
FROM Table1 a
LEFT SEMI JOIN Table2 b ON a.Name=b.Name
```
эквивалентно
```
SELECT Name
FROM Table1 a
WHERE EXISTS(
SELECT * FROM Table2 b WHERE (a.Name=b.Name))
```
...
<details>
    <summary>
        Что такое owned entity в EF Core
    </summary>
    
Owned entities (собственные типы сущностей) являются типами, которые используются только через навигационное свойство и не существуют отдельно без своего владельца.
Указываются явно через ```OwnsOne``` или ```OwnsMany```.
Для ```OwnsOne``` по-умолчанию не создается таблица, вместо этого каждое свойство сущности мапится на столбец таблицы владельца. Для ```OwnsMany``` создаётся отдельная таблица.

Собственным типам сущности не нужно определять ключ, также им не требуется указание навигационного свойства владельца, как это делается для обычных отношений.
</details>

<details>
    <summary>
        Как и когда хранить в таблице JSON
    </summary>
    
JSON в таблице моделирует собственную сущность. Он значительно упрощает ее загрузку, так как не требуется лишний джоин и индексы. Идеально подходит, когда сущность небольшая (5-10 элементов в json) и полностью обновляется с записью.

С другой стороны, у этого подхода есть ограничения:
1) Можно использовать только если сущность является собственной и не существует отдельно без владельца.
2) Не подходит, если нужна фильтрация/индексация по столбцам данной сущности.
3) Всегда оказывается загруженной, что приводит к небольшим накладным расходам даже для тех запросов, где она не используется.
</details>
...
<details>
    <summary>
    Распределенные транзакции: Saga и 2PC
    </summary>

**Saga**
В микросервисной архитектуре, когда сервисы независимы, часто возникает потребность выполнить операцию на них в рамках одной транзакции. Например, в онлайн-магазине создать запись заказа в OrderService, списать деньги за заказ в PaymentService, зарезервировать товар в WarehouseService и т.д. 

Основные принципы:
- локальные транзакции
- публикация сообщений или выполнение команд
- компенсация в случае отказа
- обязательная идемпотентность операций и компенсаций
- долговечность состояния саги

Saga бывает двух видов.
**Оркестрация** - когда существует один сервис-оркестратор, вызывающий все необходимые сервисы в нужном порядке; в случае отказа одного из компонентов выполняются компенсирующие транзакции для уже выполненых шагов.
Плюсом является то, что вся последовательность операций и все шаги хранятся в одном месте. Минус же в том, что в моменте система может находиться в неконсистентном состоянии - часть сервисов уже применила у себя изменения, а часть еще нет.
**Хореография** - когда нет единого центра управления, сервисы публикуют события. Например, после резервирования заказа в случае успеха оплаты заказа вся транзакция считается успешной, иначе публикуется сообщение PaymentFail.
Плюс - более гибкая распределенная система. Минус - логика размазана между сервисами, дебаг проблем становится затруднительным.

**2PC**

В противоположность саге с последовательностиью операций существует двухфазный коммит, где операции выполняются "транзакционно" между всеми элементами. Используется при небольшом числе участников и строгом требовании к атомарности.

Принцип работы:
- фаза подготовки - координатор рассылает всем участникам запрос на подготовку к коммиту, участники выполняют изменения, но не фиксируют их, после чего голосуют "готов" или "не готов" за общую фиксацию
- фаза фиксации - координатор рассылает всеми участникам запрос на фиксацию, участники фиксируют изменения и отправляют результат
- в случае ошибки одного из участников зафиксированные изменения откатываются

Плюсы 2PC в том, что он гарантирует строгую согласованность системы. 
Минусов много:
- блокировка ресурсов (блокировка ресурсов на время транзакции растягивается по времени самой долгой из них)
- координатор - точка отказа
- двойной round-trip на коммуникацию
- откат транзакции в случае проблем связи
...
Доставка сообщений в рэббите происходит по схеме round-robin между всеми потребителями одной очереди.
...
<details>
    <summary>
    Что такое Kafka, как она работает
    </summary>
    
Kafka - это брокер сообщений, поддерживающий семантику pull-доставки сообщений потребителям.

Основные понятия:
1) Кластер - объединение брокеров Kafka
2) Брокер - уникальная нода (сервер) Kafka
3) Топик - объект, группирующий сообщения в Kafka
4) Партиция - объект топика, используемый для распределения сообщений внутри топика
5) Продьюсер - пишущий сервис
6) Консьюмер - читающий сервисы

Сообщения в Kafka хранятся в обычных .log-файлах. У каждого файла есть предел, при достижении которого файл закрывается для записи и открывается новый. Один файл хранит сообщения одной партиции.
Сообщения состоят из offset (номер лог-файла), position (номер сообщения в файле), timestamp (назначаемая дата сообщения) и message (тела сообщения).
Файлы хранятся до истечения максимального ttl сообщения в нём.
Kafka гарантирует порядок сообщений в одной партиции.

При отправке сообщения без дополнительных опций, Kafka сама решит, в какую партицию положить сообщение (round robin).
При явном указании номера партиции сообщение попадёт в неё.
Также возможно указание ключа сообщения, тогда сообщение попадёт в партицию key_hash % n.
Последний вариант удобен тем, что позволяет складывать все сообщения с одним ключом в одну партицию и обработать их в том порядке, в каком они поступили в брокера.

Чтение из Kafka.
При подключении консьюмеров к топику можно указать GroupId, чтобы сгруппировать консьюмеров (это называется consumer group) для распараллеливания чтений.
Группа консьюмеров делит между собой все партиции топика, каждой партиции назначается ровно 1 консьюмер, при этом у консьюмера может быть больше одной партиции).
Если группа состоит из одного консьюмера, он читает все партиции.
Если группа состоит из большего числа консьюмеров, чем есть партиций, то лишние консьюмеры будут простаивать.
Разные группы независимо читают сообщения из партиций.
Для каждой группы и партиции хранится текущий оффсет, чтобы в случае падения консьюмера и назначения нового мы не перечитывали и не обрабатывали уже обработанные сообщения. Оффсет коммитится консьюмером.

Гарантии доставки.
Существует параметр acks с тремя значениями
0 - продьюсер не интересуется успехом записи (могут теряться сообщения из-за падения кластера, так как продьюсер не проверяет успех)
1- продьюсер получает подтверждения от мастер-реплики (могут теряться сообщения, если лидер упал до репликации)
-1 - продьюсер ждёт подтверждение от лидера и всех insync-реплик

Также есть два варианта коммита:
- auto commit - at most once семантика
- manual commit (после обработки сообщения) - at least once

Для того, чтобы добиться гарантии exactly once, требуется кастомный менеджмент оффсетами или идемпотентность.

Репликация в Kafka устроена по типу master-slave: в брокере для каждой партиции назначается брокер-лидер, а все остальные брокеры становятся пассивными репликами.
Лидерство распределяется между брокерами. Один и тот же брокер может быть лидером по одной партиции и репликой по другой.
Чтение и запись всегда происходит только в/из лидера.
Реплики есть синхронные и асинхронные. Через min.insync.replicas можно установить минимальное число синхронных реплик (при записи в лидера синхронно пишутся данные в реплики; так достигается гарантия того, что при выпадении лидера мы не потеряем сообщения).
...
Суть CQRS можно свести к следующему:
- разные модели (богатая модель для записи и анемичная dto для чтения)
- command ничего не возвращает
- возможность раздельной оптимизации записи и чтения
</details>

<details>
<summary>
    Elasticsearch - что это, как работает, зачем нужен
</summary>
Elasticsearch - это поисково-аналитическая система. Предназначена для работы как с полуструктурированными, так и неструктурированными данными.

Основные объекты:
1. Документ - это единица данных, которую можно сохранить и получать. Хранится в формате JSON, помимо исходных данных в документе есть служебные поля.
Не требует жесткой схемы, хотя для эффективного поиска она крайне желательна.
Каждый документ идентифицируется по служебному полю ```_id```.
2. Поле - именованное значение внутри документа с определенным типом данных.
3. Индекс - набор документов, по которому можно производить поиск.
4. Маппинг - схема, которая для каждого поля определяет тип данных (integer, text, keyword и т.д.), как это поле должно индексироваться и какие настройки индексации.
Маппинг динамический по умолчанию, то есть создаётся автоматически, но можно создать его и руками. Поменять что-то в уже созданном маппинге можно только с полной реиндексацией.
5. Инвертированный индекс - специальная структура, которая отображает список терминов на все документы, в которых они встречаются.
По умолчанию инвертированный индекс строится по всем полям документа, поэтому Elasticsearch поддерживает поиск по фразе, по числу, по двум полям и др.
Рядом с документов также хранится метаинформация в поле ```_score```, необходимая для вычисления релевантности (частота вхождения термина в документ, позиция в документе и т.д.)

Как маппинг ускоряет поиск
index: true/false - если по полю нет поиска, и его просто надо выдавать в ответе, можно исключить его из индекса для облегчения и ускорения поиска
analyzer: как разбивать текст на слова, важная настройка для типа text; правильная настройка делает поиск семантически "умным"

Как типы данных используются в поиске:
text - полнотекстовый поиск, проходит через анализатор, разбивается на термины, строится инвертированный индекс
keyword - рассматривается как отдельный термин целиком, тоже попадает в инвертированный индекс
int, float, long, date - хранятся в деревьях, позволяющих совершать поиск по диапазону за логарифмическое время
geo_point, geo_shape - специальная структура для хранения геоданных для быстрого поиска

Стоп-слова - это часто встречающиеся, но семантически "пустые" термины, которые не несут нагрузки, но раздувают индекс.
В русском языке это союзы, предлоги, частицы (и, а, но, над, под, же и т.п.)
При индексации стоп-слова не попадают в инвертированный индекс, не раздувают его и не замедляют поиск
...
<details>
<summary>
    CDC - что это и зачем нужно
</summary>
CDC (Change Data Capture) - это механизм, позволяющий отслеживать изменения данных в таблице БД (вставка, обновление, удаление) и передавать их в другие системы практически в реальном времени.

Как это работает:
1. Некий сервис пишет данные в таблицу БД
2. Задание захвата асинхронно (по отношению к записи в эту таблицу) читает журнал и записывает изменения в специальную таблицу изменений
3. Специальный сервис (как правило, Debezium, но может быть и самописный) читает данные из таблицы изменений и отправляет их в другую систему (как правило, в виде сообщений в шину)
4. Задание очистки удаляет старые записи

Минусы:
- Увеличивает нагрузку на запись в базе
- Увеличивает использование диска в базе

Что даёт:
- Позволяет в near-realtime режиме передавать данные в другие системы (Clickhouse, Elasticsearch и т.д.)
- Не нужно думать про гарантии доставки (из коробки гарантируется at least once)
- Упрощение миграций (можно дать Debezium перечитать все данные, он отправит их куда надо и автоматически перейдёт в режим реального времени)
- Снимает нагрузку отправки с пишущего сервиса (если он вдруг это делает)

Outbox, казалось бы, делает то же самое - берет сообщения из бд, отправляет их в шину. В чём разница?
- Outbox создаёт постоянную нагрузку на запись (сообщение нужно записать в рамках одной транзакции с данными), тогда как CDC читает журнал изменений
- Outbox гарантирует порядок с момента внедрения, CDC гарантирует порядок для всех данных (т.к. работает со снапшотом)
- Outbox имеет задержку доставки, равную частоте опроса воркером таблицы, у CDC же задержка минимальна
- Outbox проще в реализации, для CDC нужно усложнение инфраструктуры
- Outbox позволяет более гибко управлять событиями, отправляемыми в шину, CDC же отправляет все изменения, даже промежуточные
- Outbox работает с контрактом домена (сообщением), изменение в бд может не отразиться на потребителях;
в CDC же изменение в названии столбца таблицы может сломать потребителей

Можно использовать комбинацию Outbox + CDC. Например, подключить Debezium вместо общего журнала изменений к таблице OutboxMessages.
</details>
